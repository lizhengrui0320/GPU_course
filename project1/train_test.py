...
def test(model, loader, num_class=10):
    ...
    for j, (points, target) in enumerate(loader):
        ...
        #     file.write(pred_choice.cpu().tolist())

        choice_list = pred_choice.cpu().tolist()
...
def main():
    ...
    # 创建 DataLoader 实例
    train_dataloader = DataLoader(train_dataset, batch_size=16, shuffle=True, num_workers=4, drop_last=True, collate_fn=pad_collate_fn)
    ...
    for epoch in range(total_epoch):
        ...
        for batch_id, (points, target) in enumerate(train_dataloader, 0):
            ...
            pred_choice = pred.data.max(1)[1]                       # 产生结果
